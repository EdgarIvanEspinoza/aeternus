...
import { cacheMessages, getCachedMessages, updateCacheWithMessage } from '@utils/message-cache';
...
const ChatHook = (
  ...
  const saveMessage = async (role: 'user' | 'assistant' | 'system', content: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Actualiza la caché con el nuevo mensaje guardado
        updateCacheWithMessage(conversationId.current, {
          id: message.id,
          role: message.role,
  ...
  const resetConversation = async () => {
    ...
    localStorage.removeItem('aeternus-messages-cache');
    localStorage.removeItem('aeternus-cache-expiry');

    // Reinicializar mensajes
    setInitialMessages([]);

    // Guardar sesión nueva en el servidor
    try {
      await fetch('/api/chat/saveSession', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: user?.sub,
      ...
      // Reiniciar la página para cargar nueva sesión
  ...
  useEffect(() => {
    const initializeMessages = async () => {
      ...
      // Intenta obtener mensajes de la caché primero
      const cachedMessages = getCachedMessages(conversationId.current);
      ...
      try {
        ...
        if (data?.messages?.length > 0) {
          ...
          // Guardar en caché
          cacheMessages(conversationId.current, messages);
        }
      } catch (err) {
...
${
  ...
  const { messages, input, handleInputChange, handleSubmit, append } = useChat({
    ...
    onFinish: async (message) => {
      ...
      // Actualizar caché con todos los mensajes actuales
      if (initialMessages) {
        const allMessages = [...initialMessages, message];
        cacheMessages(conversationId.current, allMessages);
      }
    },
  });
