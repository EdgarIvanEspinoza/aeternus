    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-black text-white selection:bg-violet-500/40 selection:text-white">
      ...
      <header className="flex items-center justify-between px-6 py-5 max-w-7xl mx-auto">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-zinc-400">
          <a href="#features" className="hover:text-zinc-200 transition">
            Features
          </a>
          <a href="#principles" className="hover:text-zinc-200 transition">
            Principles
          </a>
          {/* <a
            href="/admin"
            className="rounded-full border border-violet-500/40 px-6 py-3 hover:border-violet-400/70 text-zinc-200 hover:text-white transition shadow-[0_6px_30px_-10px_rgba(167,139,250,0.35)] font-semibold">
            Admin Panel
          </a> */}
        </nav>
      </header>
      <main className="relative">
        ...
        <section id="features" className="relative mx-auto max-w-7xl px-6 pb-28">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((f) => (
              <FeatureCard key={f.title} title={f.title} highlight={f.highlight}>
                {f.body}
              </FeatureCard>
            ))}
          </div>
        ...
        <section id="principles" className="relative mx-auto max-w-5xl px-6 pb-28">
          ...
          <ul className="space-y-6 text-zinc-300 text-sm md:text-base leading-relaxed">
            <li>
              <strong className="text-white">Affective persistence:</strong> memory is not a flat list; it is a
              relational graph with dynamic weight.
            </li>
            <li>
              <strong className="text-white">Context transparency:</strong> every answer is grounded in interpretable
              states and relationships.
            </li>
            <li>
              <strong className="text-white">Safe modulation:</strong> tone and humor adapt according to emotional state
              and relational closeness.
            </li>
            <li>
              <strong className="text-white">Intentionality:</strong> conversation is not reaction; it is sustaining a
              shared narrative trajectory.
            </li>
            <li>
              <strong className="text-white">Technical elegance:</strong> minimum necessary complexity for maximum
              expressiveness.
            </li>
          </ul>
        </section>
        ...
        <section className="relative mx-auto max-w-5xl px-6 pb-32">
          <div className="relative overflow-hidden rounded-3xl border border-violet-500/30 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-900 p-10 md:p-14 shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_0_60px_-10px_rgba(168,85,247,0.35)]">
            ...
            <h3 className="relative text-2xl md:text-3xl font-semibold tracking-tight">
              Alpha Access <span className="text-rose-500">Closed</span>
            </h3>
            <p className="relative mt-4 max-w-2xl text-zinc-300 text-sm md:text-base leading-relaxed">
              The alpha testing phase has concluded. Thank you for your interest in Aeternus Lab. We are currently
              working on improvements based on the valuable feedback received during the alpha phase.
            </p>
          </div>
        </section>
      </main>
